...
pushval()
{
	...
	if (pushloop(192 , nfun, funids)) {
	    pusharg = funoffs[pusharg]+20000; /* modified by pts@fazekas.hu */
	} else if (lexval < 9 ) {
	    pusharg = 20000+lexval- 7 ; /* modified by pts@fazekas.hu */
	}
    }
    ...
    )
{
	...
	*p++ = *q++;
    ...
    putchar(0);
    putchar(0);
    if ((unsigned)n>=sizeof(code)) abort(); /**** pts ****/
    fprintf(stderr, "GLOBAL DATA SIZE=%d\n", curgloboffs); /**** pts ****/
    putchar(n);
    putchar(n/256);
    p = code;
    while(n--) {
